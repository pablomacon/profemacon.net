...
function App() {
  const isDynamicTeacherRoute = (path: string) => /^\/docente\/grupos\/\d+(?:\/(?:actividades(?:\/\d+\/resultados|\/[^/]+\/prueba)?|estudiantes\/\d+\/resultados|resultados))?$/.test(path);
  ...
  const view = route === "/" ? <Home />
    ...
    : /^\/docente\/grupos\/\d+\/estudiantes\/\d+\/resultados$/.test(route) ? (() => { const [, id, studentId] = /^\/docente\/grupos\/(\d+)\/estudiantes\/(\d+)\/resultados$/.exec(route)!; return <TeacherGroupStudentResults groupId={Number(id)} studentId={Number(studentId)} onBack={() => navigate(`/docente/grupos/${id}/resultados` as Route)} />; })()
