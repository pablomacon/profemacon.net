...
import { TeacherGroupResults } from "./teacher-group-results";
import { TeacherGroupActivityResults } from "./teacher-group-activity-results";
import { TeacherGroupStudentResults } from "./teacher-group-student-results";
import { TeacherGroupStudentAttempts } from "./teacher-group-student-attempts";
...
const supportedRoutes: Route[] = ["/", "/ingresar", "/mis-cursos", "/historial", "/docente", "/docente/importar-estudiantes", "/docente/activaciones", "/docente/actividades/variables-java-01/prueba", "/practicante", "/curso/programacion-i/unidad-0", "/curso/programacion-i/unidad-0/actividad", "/curso/programacion-i/unidad-1", "/curso/programacion-i/unidad-1/actividad/variables-java-01"];
...
function App() {
  ...
  const view = route === "/" ? <Home />
    ...
    : /^\/docente\/grupos\/\d+\/actividades\/\d+\/resultados$/.test(route) ? (() => { const [, id, activityId] = /^\/docente\/grupos\/(\d+)\/actividades\/(\d+)\/resultados$/.exec(route)!; return <TeacherGroupActivityResults groupId={Number(id)} activityId={Number(activityId)} onBack={() => navigate(`/docente/grupos/${id}/resultados` as Route)} onOpenStudent={(studentId) => navigate(`/docente/grupos/${id}/estudiantes/${studentId}/resultados` as Route)} />; })()
    : /^\/docente\/grupos\/\d+\/estudiantes\/\d+\/resultados$/.test(route) ? (() => { const [, id, studentId] = /^\/docente\/grupos\/(\d+)\/estudiantes\/(\d+)\/resultados$/.exec(route)!; return <TeacherGroupStudentResults groupId={Number(id)} studentId={Number(studentId)} onBack={() => navigate(`/docente/grupos/${id}/resultados` as Route)} onOpenActivity={(activityId) => navigate(`/docente/grupos/${id}/estudiantes/${studentId}/actividades/${activityId}/intentos` as Route)} />; })()
    : /^\/docente\/grupos\/\d+\/estudiantes\/\d+\/actividades\/\d+\/intentos$/.test(route) ? (() => { const [, id, studentId, activityId] = /^\/docente\/grupos\/(\d+)\/estudiantes\/(\d+)\/actividades\/(\d+)\/intentos$/.exec(route)!; return <TeacherGroupStudentAttempts groupId={Number(id)} studentId={Number(studentId)} activityId={Number(activityId)} onBack={() => navigate(`/docente/grupos/${id}/estudiantes/${studentId}/resultados` as Route)} />; })()
    : /^\/docente\/grupos\/\d+\/resultados$/.test(route) ? <TeacherGroupResults groupId={Number(route.split("/")[3])} onBack={() => navigate(`/docente/grupos/${route.split("/")[3]}` as Route)} onOpenActivity={(activityId) => navigate(`/docente/grupos/${route.split("/")[3]}/actividades/${activityId}/resultados` as Route)} onOpenStudent={(studentId) => navigate(`/docente/grupos/${route.split("/")[3]}/estudiantes/${studentId}/resultados` as Route)} />
    : /^\/docente\/grupos\/\d+$/.test(route) ? <TeacherGroupPage id={Number(route.split("/").at(-1))} onBack={() => navigate("/docente")} onActivities={() => navigate(`${route}/actividades` as Route)} onResults={() => navigate(`${route}/resultados` as Route)} />
