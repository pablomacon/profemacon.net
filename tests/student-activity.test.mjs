...
test("GET /api/me/activities/:slug aplica el contrato público con D1 local", async (t) => {
  ...
  const anonymousGroups = await fetch(`${baseUrl}/api/teacher/groups`);
  assert.equal(anonymousGroups.status, 401);
  assert.equal(anonymousGroups.headers.get("Cache-Control"), "no-store");
  const teacherGroups = await fetch(`${baseUrl}/api/teacher/groups`, { headers: { Cookie: `pm_session=${teacherToken}` } });
  assert.equal(teacherGroups.status, 200);
  const teacherGroupsBody = await teacherGroups.json();
  assert.deepEqual(teacherGroupsBody.groups.map((group) => group.id), [1]);
  assert.equal(teacherGroupsBody.groups[0].activeStudents, 2);
  const studentGroups = await fetch(`${baseUrl}/api/teacher/groups`, { headers: { Cookie: `pm_session=${studentToken}` } });
  assert.equal(studentGroups.status, 403);
  const ownGroup = await fetch(`${baseUrl}/api/teacher/groups/1`, { headers: { Cookie: `pm_session=${teacherToken}` } });
  assert.equal(ownGroup.status, 200);
  assert.equal((await ownGroup.json()).group.code, "grupo-a");
  const foreignTeacherDashboardGroup = await fetch(`${baseUrl}/api/teacher/groups/2`, { headers: { Cookie: `pm_session=${teacherToken}` } });
  assert.equal(foreignTeacherDashboardGroup.status, 403);
  const missingGroup = await fetch(`${baseUrl}/api/teacher/groups/999999`, { headers: { Cookie: `pm_session=${teacherToken}` } });
