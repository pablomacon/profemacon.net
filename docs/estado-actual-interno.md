...
- **6D-B PENDIENTE: consumo automático de materiales.** Registry dinámico con `import.meta.glob`, página genérica de unidad, navegación y ruteo para que un material nuevo aparezca sin editar TSX, render real de Mermaid y YouTube reutilizando `MermaidDiagram` y `VideoEmbed`, y un material real de prueba.
- Publicación de materiales por grupo y edición (etapa posterior, sobre `contenidos` y `publicaciones_contenido`): posterior a 6D-B.
- Integración de IA más profunda dentro de la plataforma: posterior.

6A, 6B, 6C y 6D-A están terminados: el Hito 6 todavía no está completo porque falta 6D-B.
