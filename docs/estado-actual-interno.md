...
- **6B PENDIENTE: render de recursos `image` y `code`** en la interfaz del estudiante y en el modo prueba docente, con retiro de la advertencia `RESOURCES_NOT_RENDERED`.
- **6C PENDIENTE: asignación y habilitación automatizada por grupos**, mediante un contrato JSON propio que mantenga separadas la actividad y su publicación.
- **6D PENDIENTE: pipeline de materiales teóricos.**
- Integración de IA más profunda dentro de la plataforma: posterior.

Sólo 6A está terminado: el Hito 6 no está completo.
