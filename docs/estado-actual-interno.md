...
**Pendiente.** `A2.1a-R1` queda **abierto**: el reintento de `0012` (una sola escritura, ya con el árbol en LF) requiere autorización humana explícita, y `A2.1a-R2` —deploy del Worker versionado— sigue detrás de él. Se mantiene la advertencia de rollback: el Worker anterior deja de ser válido en cuanto ocurra la primera revocación global real.
