...
Queda un estado transitorio explícito —**esquema nuevo con el Worker viejo**— que es seguro porque la versión desplegada ya no podía completar la derivación de 600.000 (fallaba antes de escribir) y la tabla de credenciales sigue vacía, así que no hay ninguna credencial que el `CHECK` nuevo pudiera volver ilegible. El siguiente gate es **R2**: `npm run beta:build` y deploy del código compatible; recién después se reintenta **B2.5** (`npm run beta:smoke -- https://<host-beta>`) y se mide el `cpuTime` real. **A1 todavía no está completo** y sigue prohibido importar cuentas reales.
