...
Con R2 desplegado, el estado transitorio **esquema nuevo con el Worker viejo quedó cerrado**: la versión activa ya usa el rango verificable 50.000–100.000 y el tope del runtime dejó de ser un bloqueo. El único paso pendiente de A1 es **B2.5 — recorrido de humo** (`npm run beta:smoke -- https://<host-beta>`), después del cual corresponde medir el `cpuTime` real de 100.000 iteraciones para decidir sobre Workers Paid. **B2.5 todavía no fue ejecutado y A1 sigue abierto**; sigue prohibido importar cuentas reales.
