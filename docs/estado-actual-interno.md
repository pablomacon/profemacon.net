...
- Versión de autenticación por cuenta (`0012`): `usuarios.auth_version` nace en `1` y cada sesión copia esa versión de forma inmutable al crearse, en la misma sentencia `INSERT … SELECT`. Una sesión es válida sólo si su versión es **igual** a la vigente, además de los controles anteriores; cualquier diferencia es `401`.
- Revocación global atómica (`worker/session-revocation.ts`): incrementa la versión y revoca explícitamente las sesiones vigentes en un único `db.batch()`, y audita `sessions_revoked` con metadata mínima. El incremento no lleva condición de techo: el `CHECK` decide y, si falla, el batch completo se aborta. Todavía no tiene endpoint público. El logout individual no cambia la versión.
...
- revocación global de sesiones probada de punta a punta con `wrangler dev` real y D1 temporal (`tests/auth-session-revocation.test.mjs`): `0012` aplicada sobre un esquema `0011` con datos históricos —usuarios y sesiones preexistentes versionados en `1`, datos intactos, `CHECK` de rango y dos disparadores nuevos—, creación de sesión versionada en una sola sentencia, validación por igualdad exacta de versión (diferencia en ambos sentidos → `401`), revocación global atómica con recuento exacto e idempotencia (+1 por ciclo), logout individual sin cambio de versión, `401` posterior por revocación, expiración, inactividad y cuenta suspendida o archivada, auditoría `sessions_revoked` sin secretos, cuenta inexistente sin evento fantasma y rollback completo del batch cuando el `CHECK` rechaza el incremento. Los registros del runtime no contienen la contraseña ni el token.
...
- revocación de todas las sesiones de una cuenta: la primitiva local ya existe y está probada (§15), pero todavía no tiene endpoint que la invoque;
...
## 15. A2 — Seguridad operacional (iniciado en local)

### A2.1a-L — Versión de autenticación y revocación global (sólo local)

**Alcance.** Bloque **exclusivamente local**. No se tocó Cloudflare remoto, ni la D1 remota, ni `wrangler secret`, ni seeds remotos, ni datos reales; no hubo deploy ni smoke. El remoto sigue en el esquema `0011` con el Worker de R2/B2.5.

**Estado.** Arquitectura revisada y aprobada antes de implementar. Detalles:

- **`auth_version` agregado.** La migración `0012_auth_version_sesiones.sql` agrega `usuarios.auth_version INTEGER NOT NULL DEFAULT 1 CHECK (auth_version BETWEEN 1 AND 2147483647)`. El `DEFAULT 1` cubre a los usuarios preexistentes sin backfill fila a fila. **No se usa ninguna condición de techo en el código**: es el `CHECK` el que decide y, si el incremento lo viola, el batch completo se aborta (`fail-closed`).
- **Sesiones versionadas.** `sesiones_usuario.auth_version INTEGER NOT NULL DEFAULT 1` conserva la copia tomada al crear la sesión. Dos disparadores la protegen: `trg_sesion_auth_version_coherente_insert` impide que una sesión nazca con una versión distinta de la vigente del usuario (incluso por `DEFAULT`) y `trg_sesion_auth_version_inmutable_update` impide modificarla después. No se agregó ningún índice nuevo ni se reconstruyó ninguna tabla: `ALTER TABLE … ADD COLUMN` fue suficiente.
- **Creación atómica.** `createLocalSession` (`worker/local-auth.ts`) dejó de leer la versión y volver a TypeScript: ahora una sola sentencia `INSERT INTO sesiones_usuario (…) SELECT …, u.auth_version, … FROM usuarios u WHERE u.id = ?3 AND u.estado = 'activo'`. Sin fila insertada no hay token y la ruta responde `401` sin cookie. El token sólo llega al llamador que construye la cookie y la función no se exporta.
- **Validación por versión.** `authenticateRequest` (`worker/auth.ts`) sigue siendo el único validador del Worker y ahora exige además `s.auth_version = u.auth_version`, junto con hash del token, `revocada_en IS NULL`, vencimiento absoluto, inactividad y `estado = 'activo'`. Cualquier diferencia —sesión más vieja o más nueva— es `401`. Todas las rutas protegidas de `worker/index.ts` pasan por ese helper.
- **Revocación global atómica.** `worker/session-revocation.ts` (`revokeAllSessions`) ejecuta en un único `db.batch()`: incremento de `auth_version`, revocación explícita de las sesiones vigentes (`revocada_en IS NULL AND expira_en > CURRENT_TIMESTAMP`, sin borrar filas) y auditoría; una cuarta sentencia de sólo lectura devuelve la versión resultante dentro de la misma transacción. Devuelve `{ authVersion, revokedSessions }`. La auditoría se escribe con `WHERE EXISTS`, así que una cuenta inexistente produce un error controlado (`USER_NOT_FOUND`) sin evento fantasma.
- **Logout individual preservado.** `revokeLocalSession` sigue revocando sólo esa sesión, no cambia `auth_version`, no afecta otras sesiones y no se registra como `sessions_revoked`.
- **Auditoría.** `accion = 'sessions_revoked'`, `entidad_tipo = 'usuario'`, `entidad_id` de la cuenta objetivo y `datos_json` con metadata mínima (`{"motivo": …}`); motivos previstos `admin`, `password_change`, `password_reset` y `security`. Nunca se registran token, cookie, contraseña, hashes de credencial ni IP. La marca de último uso no refresca una sesión que dejó de ser válida.
- **API pública.** No se creó ningún endpoint de cambio de contraseña, de restablecimiento ni administrativo: el bloque deja primitivas, migración, validación y pruebas. La revocación global se ejercita desde `tests/support/session-revocation-entry.ts`, una entrada de Worker **sólo para pruebas** que agrega la sonda de la primitiva y delega todo lo demás en el manejador de producción.
- **Pruebas.** `tests/auth-session-revocation.test.mjs` cubre la migración `0012` sobre un esquema `0011` con datos históricos, los dos disparadores, el `CHECK` de rango, la creación versionada, la validación por igualdad exacta en ambos sentidos, la revocación global con recuento exacto e idempotencia, las revocaciones simultáneas sin perder incrementos, el intercalado entre creación de sesión y revocación, el rollback completo del batch cuando el `CHECK` rechaza el incremento, la ausencia de eventos fantasma, la conservación de expiración, inactividad, estado y revocación explícita, el logout individual y los contratos de fuente (un único validador, una única creación versionada, sesiones sólo en los tres módulos autorizados). La limpieza de sus espacios temporales reintenta ante el `EPERM` intermitente de Windows (un handle de SQLite del estado local se libera más tarde que el proceso de Wrangler) y, si aun así no puede borrar, avisa y no falla la prueba: es el flake de entorno ya conocido, ajeno a las aserciones. `tests/local-auth-flow.test.mjs` y los contadores de migraciones se actualizaron de `11` a `12`.
- **Deuda deliberadamente fuera de alcance.** No se corrigió el tiempo de inactividad efectivo (~35 minutos nominales sobre 30 declarados): es deuda separada. No se implementó cambio ni restablecimiento de contraseña.

**Rollout remoto pendiente.** Aplicar `0012` en `profemacon-beta-remote` y desplegar el Worker versionado es un bloque posterior, con autorización humana explícita. Cuando ocurra, hay que tener presente que **el rollback al Worker anterior deja de ser válido después de la primera revocación global real**: el Worker viejo no compara versiones y consideraría vigente una sesión ya revocada globalmente.

### A2.1a-R1 — Primer intento remoto de `0012` (2026-09-24): **NO APLICADA**

**Resultado.** El bloque remoto **no se completó**. Con autorización humana explícita y una única escritura autorizada —`wrangler d1 migrations apply profemacon-beta-remote --remote --env beta` (binary local `node_modules\.bin\wrangler.cmd`, Wrangler 4.112.0), que anunció **una sola** migración pendiente, `0012_auth_version_sesiones.sql`—, la API de D1 rechazó la migración con **HTTP 400** y `incomplete input: SQLITE_ERROR [code: 7500]` (exit 1). La migración es **atómica**: no se registró en `d1_migrations` y no quedó ningún objeto parcial. No hubo reintento ni `restore`, y un primer lanzamiento que quedó detenido en el prompt interactivo `(Y/n)` tampoco escribió nada (se comprobó antes de responderlo).

**Causa: defecto conocido de la herramienta, no del SQL.** `wrangler d1 migrations apply --remote` falla con ese mensaje cuando el **archivo de migración tiene finales de línea CRLF y contiene un disparador** (`BEGIN … END;`): es `workers-sdk` **#14991**, refinado por **#15314**. La causa inmediata en este repositorio fue `core.autocrlf = true` **sin `.gitattributes`**, que dejaba `0011` y `0012` con CRLF en el árbol de trabajo, aunque el índice siempre estuvo en LF. Encaja con todo lo demás: `0001`–`0010` están en LF y `0003/0007/0008/0009/0010` aplicaron disparadores sin problema; `0011` (CRLF, sin disparadores) aplicó bien en R1; y la matriz del informe upstream marca ✅ para LF, ✅ para CRLF sin disparadores, ✅ para la ruta local y ❌ sólo para CRLF con disparadores. Comprobaciones locales del SQL: el splitter real de Wrangler sobre `0012` produce **5 sentencias completas y válidas**, y `tests/auth-session-revocation.test.mjs` aplica `0012` sin errores por la ruta local.

**Evidencia de que el remoto quedó intacto** (sólo `SELECT`/`PRAGMA`, todas con `rows_written: 0` y `changed_db: false`): `d1_migrations` = **11** (última `0011`); `PRAGMA table_info(usuarios)` y `PRAGMA table_info(sesiones_usuario)` **sin** `auth_version`; **33** disparadores, sin `trg_sesion_auth_version_coherente_insert` ni `trg_sesion_auth_version_inmutable_update`; inventario de `sqlite_master` **idéntico al baseline** (118 objetos: 29 tablas, 56 índices, 33 disparadores; `Compare-Object` = **0 diferencias**); conteos sin cambios (**2 usuarios · 1 credencial local · 1 sesión histórica · 1 grupo**); activaciones demo sin cambios (`estudiante.demo` consumida, `ana.docente` sin consumir); `PRAGMA foreign_key_check` = **0**; `size_after` 401408 → 401408. Bookmark previo `00000012-00000000-000050f0-b08ff47f120dbddc33f36f04be8a88ba` (actual `00000014-…-4c270819`, sólo por las lecturas): **no se ejecutó `restore`**.

**No se ejecutó:** deploy del Worker, seeds, `secret put`, `revokeAllSessions`, ningún cambio de `auth_version`, ningún smoke, ni login, activación o logout.

**Arreglo preparado (sólo local).** Se agregó `.gitattributes` con `migrations/*.sql text eol=lf` —el workaround recomendado por el informe upstream— y se normalizó el árbol de trabajo de `0011` y `0012` a LF; el contenido de ambas queda **idéntico** al blob ya comprometido (verificado por SHA-256), o sea que el cambio es exclusivamente de finales de línea. Además, `tests/beta-infrastructure.test.mjs` incorpora una **guarda** que exige que toda migración esté en LF y sin BOM y que la regla exista, para que la regresión no vuelva en silencio. Pruebas de este bloque: `node --test tests/beta-infrastructure.test.mjs` **21/21** y `node --test tests/auth-session-revocation.test.mjs` **10/10**.

**Cierre.** El reintento con el árbol ya normalizado a LF se ejecutó en el bloque siguiente (`A2.1a-R1-bis`) y quedó **verde**. Se mantiene la advertencia de rollback: el Worker anterior deja de ser válido en cuanto ocurra la primera revocación global real.

### A2.1a-R1-bis — Reintento único de `0012` (2026-09-24): **APLICADA**

**Resultado.** Con autorización humana explícita se ejecutó **una sola vez** el mismo comando sobre la D1 beta —`wrangler d1 migrations apply profemacon-beta-remote --remote --env beta`— con el archivo ya normalizado a LF: Wrangler anunció **una sola** migración pendiente (`0012_auth_version_sesiones.sql`), ejecutó **6 comandos** y la marcó con estado ✅ (exit 0). El balance del bloque es, por lo tanto: **un intento remoto fallido previo que fue revertido atómicamente y no dejó cambios persistentes** (`A2.1a-R1`) y este segundo intento, que sí persistió.

**Preflight.** `git status --porcelain -uall` **vacío** · `HEAD` = `origin/main` = `148a480` · `rev-list` `0 0` · guarda `--require-real` exit **0** · `git ls-files --eol migrations` = **`w/lf` en las 12** y sin BOM · `0011` y `0012` con contenido **idéntico** al blob comprometido (SHA-256) · `tests/beta-infrastructure.test.mjs` **21/21**. Bookmark pre-reintento `00000016-00000000-000050f0-b4d8b8c444266e18df4222b9f7f7c027` (registrado; **sin** `restore`).

**Estado previo (sólo `SELECT`/`PRAGMA`).** `d1_migrations` = **11** · `0012` ausente · sin `auth_version` en ninguna tabla · 2 usuarios · 1 credencial local · 1 sesión · 1 grupo · activaciones (estudiante consumida, docente sin consumir) · **33** disparadores · `foreign_key_check` = **0** · inventario **118 objetos** (29 tablas, 56 índices, 33 disparadores). Nunca se usó `migrations list --remote`.

**Verificación posterior (sólo `SELECT`/`PRAGMA`, todas con `rows_written: 0` y `changed_db: false`).** `d1_migrations` = **12**, última `0012_auth_version_sesiones.sql`; `usuarios.auth_version` (cid 9) `INTEGER NOT NULL DEFAULT 1`, con la tabla almacenada conservando `CHECK (auth_version BETWEEN 1 AND 2147483647)`; `sesiones_usuario.auth_version` (cid 9) `INTEGER NOT NULL DEFAULT 1`; disparadores = **35**, con `trg_sesion_auth_version_coherente_insert` y `trg_sesion_auth_version_inmutable_update` y sus cuerpos exactos; `ana.docente = 1` y `estudiante.demo = 1`; la única sesión histórica (`usuario_id = 2`) en **1**; **0** usuarios y **0** sesiones fuera de 1; conteos **2 usuarios · 1 credencial local · 1 sesión · 1 grupo** sin cambios; activaciones sin cambios; `foreign_key_check` = **0**; inventario **120 objetos** (29 tablas, **56 índices idénticos**, **35 disparadores**) y el diff contra el baseline es **exactamente +2 disparadores**, sin tablas ni índices nuevos ni tablas auxiliares.

**Estado transitorio vigente: esquema `0012` + Worker viejo.** La convivencia es compatible **mientras** todos los usuarios y todas las sesiones sigan en `auth_version = 1`: el Worker desplegado (R2/B2.5) no conoce la columna, pero ninguna versión cambió, así que su validación de sesión sigue siendo correcta. Todavía **no** hubo revocación global ni incremento real de versión, por lo que el **rollback al Worker actual sigue siendo posible**; deja de serlo en cuanto ocurra la primera revocación global real. No se probó la revocación, no se creó ninguna sesión nueva y no hubo login ni smoke.

**No se ejecutó:** deploy del Worker, seeds, `secret put`, `revokeAllSessions` ni ningún cambio manual de `auth_version`.

**Pendiente.** `A2.1a-R2` —deploy del Worker versionado— requiere autorización humana explícita; recién después corresponden el smoke compatible y la prueba de revocación global.
