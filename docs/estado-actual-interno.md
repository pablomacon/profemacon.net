...
Esa base remota está **vacía**: `num_tables: 0`, 0 lecturas y 0 escrituras, y las diez migraciones figuran como pendientes. El Worker beta **no está desplegado** y **no hay ningún secreto configurado**. Tampoco se aplicó ningún seed ni se cargó ningún dato, real ni ficticio: lo único que existe remotamente es el contenedor de la base.

El siguiente gate es **B2.2 — `DOCUMENT_HMAC_KEY` de la beta**: generarla nueva, distinta de la local y de la futura real, cargarla como secreto con confirmación humana y verificar su presencia sin exponer su valor. Después seguirán B2.3 (migraciones `0001`–`0010` y seed exclusivamente ficticio) y B2.4 (deploy del Worker beta y recorrido de humo). **A1 todavía no está completo** y sigue prohibido importar cuentas reales.
