...
- Completado (Bloque C, C1): matriz de resultados del grupo, con backend y frontend verificados por pruebas Worker/D1 y E2E. C2/C3/C4 (detalle de actividad, detalle de estudiante y detalle de intentos/respuestas) siguen pendientes.
...
El próximo frente técnico antes de usar datos reales es preparar la configuración remota de `DOCUMENT_HMAC_KEY`. El **Hito 4 — Actividad 1 completa** está funcionalmente cerrado en entorno local, incluido el modo prueba docente sin persistencia académica. El **Hito 5 — Panel docente mínimo** tiene cerrados el alcance por grupo (listado y resumen), el Bloque B (actividades y habilitaciones por grupo) y el corte C1 del Bloque C (matriz de resultados del grupo), todos con backend y frontend verificados. Dentro del Bloque C quedan pendientes C2 (detalle por actividad), C3 (detalle por estudiante) y C4 (detalle de intentos y respuestas); más adelante, la edición de contenido/preguntas.
