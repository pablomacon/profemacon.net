...
El intento de configurar el secreto (**B2.2**) quedó **bloqueado**: `wrangler secret list --env beta` responde `Worker "profemacon-net-2-beta" (env: beta) not found`, con la indicación de desplegar primero. Cloudflare no acepta secrets antes del primer deploy del Worker, así que el orden se reordena: **B2.3** (inventario previo y migraciones `0001`–`0010` más seed exclusivamente ficticio), **B2.4** (primer deploy del Worker beta) y sólo entonces **B2.2** (`DOCUMENT_HMAC_KEY`, generada y custodiada por el responsable fuera del agente), seguida de **B2.5** (recorrido de humo). **A1 todavía no está completo** y sigue prohibido importar cuentas reales.
