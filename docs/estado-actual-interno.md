...
El siguiente gate es **B2.2 — `DOCUMENT_HMAC_KEY`**: generarla y custodiarla fuera del agente y cargarla con `wrangler secret put DOCUMENT_HMAC_KEY --env beta`; después **B2.5** con el recorrido de humo remoto. **A1 todavía no está completo** y sigue prohibido importar cuentas reales.
